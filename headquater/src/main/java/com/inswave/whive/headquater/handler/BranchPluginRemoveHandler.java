...
@Slf4j
@Component
public class BranchPluginRemoveHandler implements Handlable {
    ...
    public void handle(WebSocketSession session, Map<String, Object> parseResult) {
        ...
        if(messageType.equals(ClientMessageType.HV_MSG_PLUGIN_REMOVE_LIST_INFO.name())){
        ...
        if(messageType.equals(ClientMessageType.HV_MSG_PLUGIN_REMOVE_LIST_INFO_FROM_HEADQUATER.name())){

            //TODO : builder 큐 관리 상태 값 업데이트 -1 적용

            String statusType = parseResult.get("message").toString();

            if(statusType.toLowerCase().equals("SUCCESSFUL")){
