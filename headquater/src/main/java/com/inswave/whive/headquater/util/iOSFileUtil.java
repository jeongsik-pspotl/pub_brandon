...
public class iOSFileUtil {
    ...
    public boolean createPlistFile(JSONObject jsonResult) throws IOException {
        ...
        log.info(qrCodeUploadDir);

        String plistWasURL = qrCodeUploadDir + "iOS/" + jsonResult.get("Project").toString() + "/"
                + jsonResult.get("createdDateTime").toString() + "/" + buildAftermanifestPlist;

        String plistWebURL = jsonResult.get("ftpUrl").toString() + "/resource/upload/iOS/"
